...
switch ($method) {
    case 'GET': {
        ...
        if (isset($_GET['id'])) {
            $stmt = $pdo->prepare('SELECT * FROM branches WHERE tenant_id = ? AND id = ? LIMIT 1');
            $stmt->execute([$tenantId, (int)$_GET['id']]);
            $row = $stmt->fetch();
            if (!$row) json_error('الفرع غير موجود', 404);
            json_ok($row);
        }
        // قائمة كاملة + إحصائية سريعة لكل فرع
        // ✅ Multi-Tenant: كل الجداول الفرعية (users/invoices) مقيّدة بنفس tenant_id
        $stmt = $pdo->prepare(
            'SELECT b.*,
                    ...
                    (SELECT COALESCE(SUM(total),0) FROM invoices i WHERE i.tenant_id = b.tenant_id AND i.branch_code = b.code
    ...
    case 'POST': {
        ...
        // إن تم تعيين فرع كرئيسي، نزيل العلم عن الباقي (ضمن متجرنا فقط)
        if ($isMain) {
        ...
        // ✅ Multi-Tenant: المفتاح الفريد الجديد هو (tenant_id, code)
        $stmt = $pdo->prepare(
        ...
        audit("upsert branch $code", $auth['email'] ?? null, 'info', $tenantId);
    ...
    case 'DELETE': {
        ...
        audit("deactivate branch $code", $auth['email'] ?? null, 'info', $tenantId);
