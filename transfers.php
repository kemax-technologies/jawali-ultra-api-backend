...
switch ($method) {
    ...
    case 'POST': {
        ...
        if ($action === 'cancel') {
            // 🔧 إصلاح جوهري (فحص شامل لنظام الصناديق والبنوك — تدقيق
            // الصلاحيات): ثغرة تصعيد صلاحيات حقيقية — واجهة Flutter
            // (transfers_screen.dart::_cancelTransfer) تمنع غير المدير من
            // إلغاء أي تحويل (store.isAdmin check)، لكن هذا المسار كان
            // يكتفي بـ require_auth() فقط، أي أن أي مستخدم مسجّل دخول (كاشير،
            // موظف مبيعات...) كان بإمكانه استدعاء
            // POST /transfers.php?action=cancel مباشرة (تجاوز الواجهة) لإلغاء
            // أي تحويل معلّق والتأثير في رصيد الصندوق. الحماية الآن على
            // مستوى الخادم أيضاً (الحارس الحقيقي/الأخير) لتطابق تماماً حارس
            // DELETE أدناه لنفس درجة الحساسية.
            if (($auth['role'] ?? '') !== 'مدير') {
                json_error('غير مصرح — إلغاء التحويلات متاح للمدير فقط', 403);
            }
            $id = trim($body['id'] ?? '');
